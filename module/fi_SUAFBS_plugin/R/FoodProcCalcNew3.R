...
foodProcessingComputation <- function(SUAinput, treeNewER, primary){
  ...
  data_compute131tree <- merge(data_compute131, treePrimary, 
                               by=c("geographicAreaM49_fi", "timePointYears", "child"))
  ...
  if(nrow(secondLevelProcessing) > 0) {
    ...
    #######################################################################
    # Consider all ICS product that can be parent and expand grid where there is a jump
    grid <- expand.grid(rank = 1:4, timePointYears = unique(toDeviateCovered$timePointYears),
                        parent_primary = unique(toDeviateCovered$parent_primary),
                        geographicAreaM49_fi = unique(toDeviateCovered$geographicAreaM49_fi))
    secondary_tree_needed <- unique(treeNewER[rank != 0 & 
                                                parent %in% treeNewER[parent %in% 
                                                                        unique(toDeviateCovered$parent_primary)]$child,
                                              .(parent, rank)])
    setnames(secondary_tree_needed, 'parent', 'parent_secondary')
    primary_tree_needed <- unique(treeNewER[rank == 0 & child %in% secondary_tree_needed$parent, 
                                            .(parent, child)])
    setnames(primary_tree_needed, c('parent', 'child'), c('parent_primary', 'parent_secondary'))
    complete_tree_needed <- merge(secondary_tree_needed, primary_tree_needed, by = 'parent_secondary', all.x = T) 
    
    expandedTree <- merge(grid,
                          complete_tree_needed, by = c('parent_primary', 'rank'), all = T)
    
    years_needed <- toDeviateCovered[,.N, c('geographicAreaM49_fi', 'timePointYears', 'parent_primary')]
    
    complete_tree <- merge(years_needed[,.(geographicAreaM49_fi, timePointYears, parent_primary)],
                           expandedTree, by =  c('geographicAreaM49_fi', 'timePointYears', 'parent_primary'))
    setkey(toDeviateCovered)
    complete_tree_proc <- merge(complete_tree, unique(toDeviateCovered[,.(geographicAreaM49_fi,
                                                                   timePointYears,
                                                                   parent_primary, secondLevelProcessing)]),
                                by = c('geographicAreaM49_fi', 'timePointYears', 'parent_primary'))
    
    secondaryFP <-  merge(complete_tree_proc, toDeviateCovered,
                          by = c('geographicAreaM49_fi', 'timePointYears', 'parent_primary', 'parent_secondary',
                                 'secondLevelProcessing'),
                          all.x = T)
    setnames(secondaryFP, 'parent_secondary', 'parent') 
    secondaryFP[is.na(availability_secondary_primEq), availability_secondary_primEq:= 0]
