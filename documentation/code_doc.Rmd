---
title: "'fi_SUA-FBS' plugin code documentation"
...
SUAwithEr <- eRcomputation(SUAwithProd, tree)
...
SUAunbal <- rbind(SUAinput[!is.na(Value), ], FPdata)
...
- Complete Global production and commodity dataset are loaded for the selected country and years. 

_ Global production is aggregated by fisheriesCatchArea variable. 

- Global production data are aggregated by ICS code according to a datatable extrated from the YBKlang file.

- The same happens with the commodity datatset but data are deviated according to the link table and the other uses datatables (provided by the technical unit).

- Once both initial datasets are aggregated at ICS cde level the SUA unbalanced is ready.

## SUA balanced

To fill the SUA with input, food processing and balancing elements figures the following steps are undertaken:

- First of all any official food processing data (code 5023) is taken aside.

- Availability is calculated for each country, year and ICS pruduct.

- Primary negative imbalances are stored and will be notified at the end of the process

- Secondary negative imbalances are stored in a dataset 'tel-quel' and then production element of the secondary products with negative imbalance is increased up to have null availability.

- Extraction rates, input and food processing are calculated using the commodity tree extrapolated from a file provided by the techincal unit. 
Functions:

  1. Extraction rate: Production and Input data are separated and their ratio (Value_prod/Value_input) is calculated by country, year and ICS secondary products. If the ratio has not been calculated because the input figure was not available the corresponding default extraction rate (included in the commodity tree) is assigned. All extraction rate figures are flagged as (E, i), i.e. estimated and calculated as an identity. A new commodity tree is built with the new extraction rates.
  
  2. Input figure: Production and extraction rates data are separated and the rario is calculated (Value_prod/Value_Er) giving the input figures. As with the extraction rates the input calculated are flagged (E,i). Eventually calculated values are integrated to the official ones.
  
  3. Food processing: now that the input figures are all available the input calculated are aggregated according to their primary parents. The total input is compared with the primary availability. If the primary availability is greater than the input to cover the processing is set equal to the total input (so everything is assigned to the primary product). If the primary availability is lower than the input then the processing for the primary is set equal to its availability and the uncovered input will be assigned to secondary parent processing. As there is a hierarchy among the products, i.e. for the order to assign input to secondary parent, the commodity tree assign to each ICS product a rank number indicating in what order the secondary parent must be considered. Primary parent have rank 0, others go from 1 to 4. A first check is performed to see if the primary parent availability can cover at least the input of the firs secondary parent. If not a message appears and problematic data are saved. The total secondary availability is converted into primary equivalent and compared to the remaining input to cover. If there is not enough secondary availability a message is created and problematic data stored. Then rank by rank the availability is converted into primary equivalent, compared with the input to cover and then, as with primary parents, if the availability is greater than the input to cover the processing is set equal to the total input (so everything is assigned to the primary product), if the availability is lower than the input then the processing for the is set equal to its availability and the uncovered input will be assigned to the next rank parent, always making sure that the availability is able to cover at least the next rank parent. All inconsistencies are stored with data and a message.

- Food processing figures are compared with the official figures previously stored and official figures replace the calculated ones.


- Balancing process: a balancing element table is stored in SWS with country specific balancing elements and they are assumed not to change across time. After food processing calculation. The availability is calculated again and the values of the balancing items are set equal to the availability. Balancing items are flagged (E, b) and availability is calculated again. If the result is not zero it means that something went wrong and a message is created along with a datatable in SWS. If a balancing element has negative value the value is set equal to zero but not included in the dataset (so the imbalance should be found in the shiny).

- Nutrients are computed from food elements available.

- Population data are loaded and 'per caput' elements calculated.

## FBSs

- Data are converted into primary equivalent and aggregated according to the provided document into FIAS and FAOSTAT FBS models.
